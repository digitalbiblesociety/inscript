...
test.describe('default Bible windows', () => {
  ...
  test('the reference focus ring stays inside the adjacent previous-version arrow', async ({ page, appPath }) => {
    await page.goto(appPath);
    await expect(page.locator('.BibleWindow .section').first()).toBeVisible({ timeout: 30_000 });

    const bibleWindow = page.locator('.BibleWindow').filter({ visible: true }).first();
    const nav = bibleWindow.locator('.text-nav');
    const cycler = bibleWindow.locator('.version-cycler');
    await cycler.evaluate(element => element.classList.add('has-versions'));
    await expect(cycler.locator('.version-prev')).toBeVisible();
    await nav.focus();

    const focusStyle = await nav.evaluate(element => ({
      visible: element.matches(':focus-visible'),
      offset: getComputedStyle(element).outlineOffset,
      width: getComputedStyle(element).outlineWidth,
    }));
    expect(focusStyle).toEqual({ visible: true, offset: '-2px', width: '2px' });
  });
});
